...
namespace tewd.Proj
{
	public class ExampleWhipProjectile : ModProjectile
	{
		...
		public override bool PreAI() {
			...
			if (!owner.channel || ChargeTime >= 10) {
				return true; // Let the vanilla whip AI run.
			}

			if (++ChargeTime % 2 == 0) // 1 segment per 12 ticks of charge.
				Projectile.WhipSettings.Segments++;

			// Increase range up to 2x for full charge.
			Projectile.WhipSettings.RangeMultiplier += 1 / 10f;
		...
		private void DrawLine(List<Vector2> list) {
			...
			Vector2 origin = new Vector2(frame.Width / 1, 1);
